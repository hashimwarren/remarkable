...
You can also focus the work with requests such as `$remarkable premise`, `$remarkable outline`, `$remarkable draft`, `$remarkable headline`, `$remarkable opening`, `$remarkable proof`, `$remarkable framework`, `$remarkable closing`, or `$remarkable critique`. Exact command syntax is not required; `prove` and `ending` remain supported aliases.
...
The writer chooses the premise and approves consequential decisions. Remarkable keeps a durable `PREMISE.md`, then turns the approved outline into a rhetorical contract for how the opening, turn, proof, framework, and closing should move the reader. Drafting, Slopless, and critique must preserve those choices or ask before changing direction. Remarkable also treats truth as a constraint and coordinates headlines, research, and informational images around the same governing premise.
